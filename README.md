...
Currently, Zero Install also maintains a list of trusted keys, and aids the user's decision of whether to trust a key using its database of known keys. The default database provides only low security; users / organisations may wish to provide their own database and perform real verification checks on authors. This would verify developers to packages that they have developed. 

Note: In the diagram the release role is a dummy role as the XML feed file takes care of what the release role was intended for, so the release role which is signed by the online key does not perform any function. 


METADATA MANAGEMENT
-------------------

The TUF metadata is going to be hosted on the 0install feed website. The user will be downloading both the XML and the TUF metadata for downloading the pacakges on 0install. 
...
Expiery of the metadata:
The metadata will have to be regenerated by 0install every fort night (15 days). If the metadata is not generated peridocially it will expire after a month. If the metadata expires the users will not be able to download the packages. 
...
At present the develpers are not validated by 0install. There is no way to findout who has developed the package, which means an attacker can create a vulnerbale package, host and distribute the package to users. 
...
Also, there is no process where a developer claims a package. In the future, when a developer creats a new package for 0install he calims the rights for that package, which mean that no other group or developer who can create the same package. By doing this we can be rest assured that an attacker will not be able to develop vulnerbale packages that have been hosted by 0install. 
...
Developer's will also have to claim package that are are currently buliding. There by only that developer or group of developer will have the rights to make and push changes to the packages they have claimed. 
...
There are two main problems that arrise here with the implementation of 0install and TUF
...
We are not too conserned about this, as current there are roughly 80 ood applications on the feeds URL and the 0install update every fortnight should not take a long time. So if a user is interrupted in the process of updating a package he can just restart the process. 
...
The overhead that is created by implementing zero install with TUF is very negligible, right now the XML file that the user has to download to launch a package is around 2.5KB. We also calculated that the average size of the TUF metadata is 2 KB or 2.5 KB per package so the total size of the file that the user would have to downlaod would be 2.5 KB + 2 KB * n where n is number of packages. With current 150 application 302.5 KB will have to be downloaded. 
